...
module.exports = async function handler(req, res) {
  ...
  try {
    const { provider, wallet, contract, contractAddress } = getVaultClient();
    ...
    let protection = {
      configured: Boolean(process.env.IEUM_PROTECTION_CONTRACT_ADDRESS),
      ok: false
    };
    ...
    if (process.env.IEUM_PROTECTION_CONTRACT_ADDRESS) {
      try {
        ...
        protection = {
          ...
          operatorMatchesWallet: operator.toLowerCase() === p.wallet.address.toLowerCase(),
        ...
        protection = {
          configured: true,
          ok: false,
          error: e.message || '생전 보호 상태 확인 실패'
        };
      }
    }
