...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const tx = await contract.requestRelease(Number(protectionId), requestHash); const receipt = await tx.wait();
    return res.status(200).json({ ok:true, txHash:receipt.hash || tx.hash });
  } catch(e) { console.error(e); return res.status(500).json({ error:e.shortMessage || e.reason || e.message || '사용 요청 기록 실패' }); }
};
